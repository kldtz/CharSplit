    ...
    if pat4list:
        for pat1 in pat4list:
            ...
            for oldpat in result:
                if mathutils.start_end_overlap((oldpat[1], oldpat[2]),
                                               (start_offset + pat1.start(1),
                                                start_offset + pat1.end(1))):
                    is_overlap = True
            if not is_overlap:
                result.append((pat1.group(1),
                               start_offset + pat1.start(1),
                               start_offset + pat1.end(1)))
