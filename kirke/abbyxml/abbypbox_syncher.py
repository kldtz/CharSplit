...
def find_aligned_abline_in_pbox_strs(um_abline: UnmatchedAbbyLine,
    ...
    for i, extra_se in enumerate(pbox_extra_se_list):
        yyy, (start, end), text, unused_ignore = extra_se
        print("    extra_pbox_str #{}: {} ({}, {}) [{}]".format(i, yyy, start, end, text))
    ...
    for xypair, (to_start, unused_to_end), text in xy_se_str_list:
        abby_pbox_offset_mapper = AlignedStrMapper(abline_st,
                                                   text,
                                                   to_start)
        if abby_pbox_offset_mapper.is_aligned:
    ...
    for ab_line in ab_line_list:
        ...
        if pbox_strinfo:
            ...
            if asmapper.is_aligned:
                ...
                if IS_DEBUG_SYNC:
                    print('ab_line not found in pdfbox by x, y:')
    ...
    if unused_pbox_xy_list:
        ...
        for unused_j, xypair in enumerate(unused_pbox_xy_list):
            ...
            if xypair2 in found_extra_xypair:
                # logger.debug("  --found pdfbox str #%d: xy=%r, %r [%s]", j, xypair, se_pair, stext)
                pass
            else:
                logger.debug("--unused pdfbox str in page %i: xy=%r, %r [%s]", pbox_page.page_num, xypair, se_pair, stext)
                check_unused_pbox_str_list.append(tmp_t3)
                after_filter_unused_pbox_xy_list.append(xypair)
        unused_pbox_xy_list = after_filter_unused_pbox_xy_list
    ...
    if page_unmatched_ablines:
        ...
        for um_abline in page_unmatched_ablines:
            ...
            if xy_fromto_se_lists_um_abline:
                ...
                logger.info("found solution for um_abline 1: %s", str(um_abline))
            else:
                ...
                if xy_fromto_se_um_abline:
                    ...
                    if xypair[0] != -1:
                        to_remove_unused_pbox_xy_set.add(xypair)
                    um_abline_fromto_selist_map[um_abline].append(fromto_se_pair)
                    logger.info("found solution for um_abline 2': %s", str(um_abline))
