...
class BigramTransform:
    ...
    def storeBigramMatrixTest(self, input_file):
        ...
        short_provisions = ['title', 'date', 'effectivedate', 'sigdate']
        ...
        for sentIdx, sent in enumerate(sentences):
            ...
            contains_dots = "..........." in sent
            num_word_ratio = num_numeric / num_words
            num_lower_ratio = num_numeric / num_words
            is_toc = num_words > 60 and num_numeric / num_words > .2
            is_table_row = num_words > 5 and num_word_ratio > .3
            if pattern and pattern.search(sent) and num_words > min_pattern_override_length and not is_toc:
                overrides[sentIdx] = 1
            if num_words < global_min_length and self.provision not in short_provisions:
                overrides[sentIdx] = 0
            if is_table_row or contains_dots:
                overrides[sentIdx] = 0

        bigram_matrix = sparse.csr_matrix((data, indices, indptr), shape=(len(sentences),
                                                                          len(self.vocabulary)),
                                          dtype=int)
    ...
    def loadSentences(self, attrvec_list):
        sentences = []
        sentences_positive = []
        for attrvec in attrvec_list:
            ...
            if self.provision in attrvec[ebattrvec.LABELS_INDEX]:
