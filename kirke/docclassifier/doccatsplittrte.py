...
def split_doccat_trte(file_name: str):
    doc_tags_list = []
    tag_freq = defaultdict(int)  # type: DefaultDict[str, int]
    # this should be type: List[Tuple[str, str]], but itertoos.combination()
    # returns Tuple[Any, ...]
    tag_cooccur_list = []  # type: List[Tuple]

    test_list = []  # type: List[str]
    train_list = []  # type: List[str]
    train_cat_count = defaultdict(int)  # type: DefaultDict[str, int]
    test_cat_count = defaultdict(int)  # type: DefaultDict[str, int]
    all_list = []  # type: List[str]
    doccat_count = defaultdict(int)  # type: DefaultDict[str, int]
    ...
    with open(file_name, 'rt') as fin:
        for line in fin:
            ...
            if tags:
                ...
                num_skipped += 1
            num_files += 1
    print("read {} files; skipped {} files because they have no tags".format(num_files,
                                                                             num_skipped))
    ...
    print("wrote {}, size= {}".format(doccat_all_fn, len(all_list)))
    ...
    with open(tmp_file_name, 'wt') as fout:
        ...
        for vval, kval in train_view:
            print("{}\t{}".format(kval, vval), file=fout)
    ...
    with open(tmp_file_name, 'wt') as fout:
        ...
        for vval, kval in doccat_view:
            print("{}\t{}\t{}\t{}".format(kval,
                                          vval,
                                          train_cat_count[kval],
                                          test_cat_count[kval]), file=fout)
        ...
        for tag, tag_count in sorted(tag_freq.items(), key=operator.itemgetter(1), reverse=True):
            if train_cat_count[tag] >= 5 and \
               test_cat_count[tag] >= 2:
                ...
                print("  skipped category '%s' because number of train doc (%d < 5) or "
                      "test doc (%d < 2)" %
