...
US_ZIP = '\d{5}(-\d{4})?'
UK_STD = '[A-Z]{1,2}[0-9ROI][0-9A-Z]?\s*(?:(?![CIKMOV])[A-Z]?[0-9O][a-zA-Z]{2})'
CAN_STD = '[A-Z][0-9][A-Z]\s+[0-9][A-Z][0-9]'
ZIP_CODE_YEAR = re.compile(r'\b\d{4,5}\b' + r'|\b{}\b'.format(UK_STD))

LOCS = KEYWORDS['uk'] + KEYWORDS['us'] + KEYWORDS['can'] + KEYWORDS['country_names'] + ['London', 'LONDON']
